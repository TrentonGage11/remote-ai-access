...
- In Docker mode, agents can pass `dockerImage` to `run_terminal`, `run_build`, or `POST /api/files/terminal` to request an allow-listed container image for that one command. Built-in choices include Node, Ubuntu, Alpine, Emscripten, dockcross Linux/Windows cross-build images, Rust musl cross builds, LLVM/Clang, .NET SDK, `osxcross`, and `valgrind` images.
