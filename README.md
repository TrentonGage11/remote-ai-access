...
- In Docker mode, agents can pass `dockerImage` to `run_terminal`, `run_build`, or `POST /api/files/terminal` to request an allow-listed container image for that one command. Built-in choices include `node:20-bookworm`, `dockcross/linux-x64`, `dockcross/linux-arm64`, `dockcross/linux-armv7`, `dockcross/windows-static-x64`, `dockcross/windows-shared-x64`, `dockcross/windows-static-x86`, `dockcross/windows-shared-x86`, `dockcross/manylinux_2_28-x64`, and `mstorsjo/llvm-mingw`.
